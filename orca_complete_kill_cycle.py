...
class OrcaKillCycle:
    ...
    def _scan_binance_market(self, min_change_pct: float, min_volume: float) -> List[MarketOpportunity]:
        ...
        # Prefer unified cache to avoid REST bans
        if UNIFIED_CACHE_AVAILABLE and get_market_cache:
            try:
                cache = get_market_cache()
                cache_tickers = cache.get_all_tickers()
                for base, t in cache_tickers.items():
                    try:
                        price = float(t.price or 0)
                        change_pct = float(getattr(t, "change_24h", 0) or 0)
                        volume = float(getattr(t, "volume_24h", 0) or 0)
                    except Exception:
                        continue
                    if price <= 0:
                        continue
                    if abs(change_pct) < min_change_pct or volume < min_volume:
                        continue

                    pair = (t.pair or f"{base}USDT").upper()
                    quote = None
                    for q in ['USDT', 'USDC', 'BUSD', 'USD', 'BTC', 'ETH', 'BNB', 'EUR', 'GBP']:
                        if pair.endswith(q):
                            quote = q
                            break
                    if not quote:
